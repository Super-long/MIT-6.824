...
import "MapReduce/6.824/src/labrpc"
import "log"
import "sync"
import "testing"
import "runtime"
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		for i := 0; i < cfg.n; i++ {
			temp := cfg.rafts[i]
			fmt.Printf("序号: %d ,currentTerm %d, votedFor %d, voteCount %d, commitIndex %d, LastApplied %d\n",i,temp.currentTerm,temp.votedFor,temp.votes_counts,temp.commitIndex,temp.lastApplied)
			if cfg.connected[i] {
		...
		for t, leaders := range leaders {
			fmt.Printf("Leader的序号是: %d\n",leaders[0])
			temp := cfg.rafts[leaders[0]]
			for i:=0;i<len(cfg.rafts);i++{
				fmt.Printf("nextIndex[i]->%d, matchIndex[i]->%d\n",temp.nextIndex[i], temp.matchIndex[i])
			}
			fmt.Println()
			if len(leaders) > 1 {
